...
} from "./fixtures/use-mutation.marko";
import UseMutationWithoutContent from "./fixtures/use-mutation-without-content.marko";
...
describe("use-mutation tag", () => {
  ...
  test("returns mutate without requiring content", async () => {
    ...
    await render(UseMutationWithoutContent, {
      client: () => client,
      mutation: () => GREETING_MUTATION,
    });
    await fireEvent.click(
      screen.getByRole("button", { name: "Mutate without content" }),
    );

    await waitFor(() => expect(observers).toHaveLength(1));
    observers[0].next({ data: { setGreeting: "Hello without content" } });
    observers[0].complete();
  });
  ...
  test("logs and rejects when the client getter returns undefined", async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      "<use-mutation> requires a client input.",
    );
    expect(
      await screen.findByText("Error: <use-mutation> requires a client input."),
    ).toBeTruthy();
  ...
  test("logs and rejects when the mutation getter is missing", async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      "<use-mutation> requires a mutation input.",
    );
    expect(
      await screen.findByText(
        "Error: <use-mutation> requires a mutation input.",
      ),
    ).toBeTruthy();
    consoleError.mockRestore();
  });
