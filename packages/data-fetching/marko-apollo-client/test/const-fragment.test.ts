...
import ConditionalConstFragment from "./fixtures/conditional-const-fragment.marko";
import ConstFragment, {
  type DogFragment,
} from "./fixtures/const-fragment.marko";
import ConstFragments from "./fixtures/const-fragments.marko";
import ConstNullFragment from "./fixtures/const-null-fragment.marko";
...
describe("const-fragment tag", () => {
  ...
  test("publishes a stable partial result when from is null", async () => {
    ...
    await render(ConstNullFragment, {
      client: () => client,
      fragment: () => DOG_FRAGMENT,
    });
  ...
  test("replaces the cache watch when from changes", async () => {
    ...
    const result = await render(ConstFragment, {
      client: getClient,
  ...
  test("unsubscribes when the tag is removed", async () => {
    ...
    const result = await render(ConditionalConstFragment, {
      client: getClient,
      fragment: getFragment,
      from: "Dog:1",
      show: true,
  ...
  test("logs a diagnostic when the fragment getter is missing", async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      "<const-fragment> requires a fragment input.",
    );
    expect(await screen.findByText("Partial")).toBeTruthy();
  ...
  test("logs a diagnostic when from is missing", async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      "<const-fragment> requires a from input.",
    );
    expect(await screen.findByText("Partial")).toBeTruthy();
    consoleError.mockRestore();
  });
  ...
  test("logs a diagnostic when the browser client is unavailable", async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      "<const-fragment> requires a client input.",
    );
    expect(await screen.findByText("Unknown dog")).toBeTruthy();
    expect(screen.getByText("Partial")).toBeTruthy();
    consoleError.mockRestore();
