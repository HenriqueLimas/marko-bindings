...
import UseSelectedQuery from "./fixtures/use-selected-query.marko";
...
describe("use-query tag", () => {
  ...
  test("logs and publishes an error when the query getter is missing", async () => {
    ...
    // @ts-expect-error Exercise the runtime diagnostic for invalid callers.
    await render(UseQuery, { client: () => client, query: {} });
  ...
  test("reacts to cache updates", async () => {
    ...
    await render(UseQuery, { client: getClient, query });
  ...
  test("unsubscribes from the query when the tag is removed", async () => {
    ...
    const rendered = await render(ConditionalUseQuery, {
      client: getClient,
      query,
      show: true,
    });
