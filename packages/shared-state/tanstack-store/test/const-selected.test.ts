...
import { createStore } from "@marko-bindings/tanstack-store";
import { afterEach, describe, expect, test, vi } from "vitest";

import ConditionalConstSelected from "./fixtures/conditional-const-selected.marko";
import ConstStore from "./fixtures/const-store.marko";
import ConstSelectedIdentity from "./fixtures/const-selected-identity.marko";
import ConstSelected from "./fixtures/const-selected.marko";

afterEach(cleanup);

const selectCount = (state: { count: number }) => state.count;

describe("const-selected tag", () => {
  ...
  test("returns the selected store value", async () => {
    ...
    await render(ConstSelected, { store, selector: selectCount });
  ...
  test("defaults to selecting the complete store value", async () => {
    ...
    await render(ConstSelectedIdentity, { store });
  ...
  test("updates when the selector changes", async () => {
    const store = createStore({ count: 1, label: "one" });
    const result = await render(ConstSelected, {
      store,
      selector: selectCount,
    });
  ...
  test("unsubscribes when the tag is removed", async () => {
    ...
    const result = await render(ConditionalConstSelected, {
      store,
      show: true,
    });
