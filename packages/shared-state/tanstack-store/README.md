# @marko-bindings/tanstack-store

TanStack Store and atom bindings for Marko 6.

The package owns its `@tanstack/store` dependency and re-exports its public API, so applications only need to install `@marko-bindings/tanstack-store`.

## Install

```sh
pnpm add @marko-bindings/tanstack-store
```
...
```marko
import { createAtom } from "@marko-bindings/tanstack-store";
...
```marko
import { shallow } from "@marko-bindings/tanstack-store";
...
import {
  ...
  createStore,
  flush,
} from "@marko-bindings/tanstack-store";
```
