...
- a conditional guest field whose TanStack lifecycle follows its markup;
- an indexed question array with add, move, and remove actions; and
...
the field again. The questions section uses `mode="array"` and indexed
`<const-field>` declarations so structural actions retain the correct values and
field metadata.
